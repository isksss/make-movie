...
- GPU backend は wgpu による frame clear と、Image layer の矩形 texture 合成、mask、fit、opacity、rotation、transition、Video / Text / Subtitle layer の raster texture 合成に対応済みです。未対応 layer は既存 CPU 合成へフォールバックします。
